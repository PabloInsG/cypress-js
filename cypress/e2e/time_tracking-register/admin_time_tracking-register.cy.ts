...
describe("Checking the employes working time as admin", () => {
    ...
    it("data should be displayed correctly in the tracking time table", () => {
        ...
        cy.intercept("GET", "https://api-crm.dev.iberley.org/api/element_registries/qr?page=1&itemsPerPage=10&properties%5B0%5D=right.empleados.nombre&properties%5B1%5D=right.empleados.Grupo&properties%5B2%5D=tipo&properties%5B3%5D=ubicacion&properties%5B4%5D=fecha_creacion&properties%5B5%5D=id_creador&return_totals=true")
            .as("consultaRegistros");

        cy.enterOfficeAndLogin(office, user.userName, user.userPassword);
        cy.clickOnModuleElement("Ficheros", "Registro Horario");
        ...
        cy.wait("@consultaRegistros").then((response) => {
            ...
            cy.get(".el-table__body tbody tr").each(($tr, index) => {
                ...
                cy.tracking_hour_admin(data, tds, todayDay, todayMonth, todayYear, index);
            });
        });
    });

    // OK. IT needs to be reviewed because the track clock doesn´t work properly
    it("an IN register should be added once a workday starts", () => {
        ...
        cy.wait("@workdayRegister").then((response) => {
            ...
            cy.get(".el-table__body tbody tr").eq(0).then(($tr) => {
                ...
                // We check the columns
                cy.tracking_hour_admin(data, tds, todayDay, todayMonth, todayYear, 0);
            });
        });
