...
import { dashElement } from 'cypress/lib/utils';

describe('test expected breadcrumbs', () => {
  ...
  context('Test to confirm if modules have expected breadcrumbs', () => {
    ...
    officeModules.forEach(officeModule => {
      it(`Test module ${officeModule} - breadcrumbs`, () => {
        ...
        cy.testOverModulElements(module, 'view/list/', body => {
          cy.findByTestId('breadcrumb_item__' + dashElement(module.label));
          cy.findByTestId('breadcrumb_item__' + dashElement(body.id));
        });
      });
    });
  });
